...
/*   Updated: 2023/02/01 12:35:56 by eduardo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	smallest_stack_b25(t_list **stack_a, t_list **stack_b)
{
	...
	{
		if (get_pos(smallest(*stack_b), *stack_b) >= 15)
		{
			while ((*stack_b)->box > smallest(*stack_b))
				rrb(stack_b);
			pa (stack_b, stack_a);
		}
		else if (get_pos(smallest(*stack_b), *stack_b) < 15)
		{
			while ((*stack_b)->box > smallest(*stack_b))
				rb(*stack_b);
			pa (stack_b, stack_a);
...
void	smallest_stack_a25(t_list **stack_a, t_list **stack_b)
{
	...
	{
		if (get_pos(smallest(*stack_a), *stack_a) >= 15)
		{
			while ((*stack_a)->box > smallest(*stack_a))
				rra(stack_a);
			pb (stack_a, stack_b);
		}
		else if (get_pos(smallest(*stack_a), *stack_a) <= 14)
		{
			while ((*stack_a)->box > smallest(*stack_a))
				ra(*stack_a);
			pb (stack_a, stack_b);
...
void	smallest_stack_a50(t_list **stack_a, t_list **stack_b)
{
	while (ft_lstsize(*stack_b) != 100)
	{
		if (get_pos(smallest(*stack_a), *stack_a) >= 25)
		{
			while ((*stack_a)->box > smallest(*stack_a))
				rra(stack_a);
			pb (stack_a, stack_b);
		}
		else if (get_pos(smallest(*stack_a), *stack_a) < 25)
		{
			while ((*stack_a)->box > smallest(*stack_a))
				ra(*stack_a);
			pb (stack_a, stack_b);
